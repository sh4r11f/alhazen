"""Declarative plots understood by the live monitor in the browser.
...
:mod:`alhazen.live_monitor.panels`, so every task's live monitor is read the same
...
# Which group of the live monitor each kind belongs to. Panels are read in
...
# a long live monitor stays something an experimenter can take in.
...
# because every factor adds two panels and a live monitor nobody can take in
# at a glance is not monitoring. Declare more explicitly when you want them.
CONDITION_PANEL_LIMIT = 2


class LiveMonitorPanel(Model):
    ...
    def _fields_for_kind(self) -> LiveMonitorPanel:
...
DEFAULT_PANELS = (
    LiveMonitorPanel(kind="performance", title="Performance"),
    LiveMonitorPanel(kind="rewards", title="Reward earned"),
    LiveMonitorPanel(kind="outcomes", title="Outcomes"),
    LiveMonitorPanel(
        kind="histogram",
        title="Reaction time",
        value="rt_ms",
    ...
    LiveMonitorPanel(kind="responses", title="Responses", value="response_key"),
    LiveMonitorPanel(
        kind="scatter",
...
def _condition_panels(field: str) -> tuple[LiveMonitorPanel, ...]:
...
class LiveMonitorSpec(Model):
    """A task's live monitor additions; defaults may be retained or replaced."""

    include_defaults: bool = True
    panels: tuple[LiveMonitorPanel, ...] = ()

    def resolved_panels(self, condition_fields: Sequence[str] = ()) -> tuple[LiveMonitorPanel, ...]:
        ...
        live monitor reflects the experiment that ran rather than one declared in
...
def _coloured_by(panel: LiveMonitorPanel, field: str | None) -> LiveMonitorPanel:
